...
class TestAccreditationAnalytics(DmaAccreditationCommon):
    ...
    def setUpClass(cls):
        ...
        # exactly the fixture: "the median wait is 48 hours" is only checkable
        # when nothing else is in the sample, and a demo database ships a year
        # of generated history that lands squarely in these date windows.
        #
        # The isolation is a *company*, not an archive flag. Archiving hides a
        # file from the live half of the analytics but not from the historical
        # half - which is deliberate, see the note on the module - whereas the
        # module's own record rules scope both halves by company. So the
        # fixture gets a directorate of its own, and the figures are read by a
        # user who can only see it. That also means these tests exercise the
        # analytics through the record rules rather than around them.
        cls.company = cls.env["res.company"].create({"name": "DMA Analytics Fixture"})
        cls.role_users = (
            cls.user_reception | cls.user_gd | cls.user_legal | cls.user_cert
            | cls.user_operations | cls.user_finance | cls.user_committee
            | cls.user_manager
        )
        cls.role_users.write({
            "company_ids": [(4, cls.company.id)],
            "company_id": cls.company.id,
        })

    def _new_request(self, **values):
        """Every fixture file belongs to the isolated directorate."""
        values.setdefault("company_id", self.company.id)
        return super()._new_request(**values)

    @property
    def Analytics(self):
        """The analytics as the Accreditation Manager of the fixture company."""
        return self.env["dma.accreditation.request"].with_user(
            self.user_manager
    ...
    def test_03_the_percentile_aggregate_reaches_postgresql(self):
        ...
        rows = self.env["dma.approval.line"].with_user(self.user_manager).with_company(self.company)._read_group(
            domain=[
    ...
    def test_04_stage_durations_are_measured_per_visit(self):
        ...
        with self._at("2026-03-20 08:00:00"):
            data = self.Analytics.get_process_performance_data(
    ...
    def test_13_the_sla_payload_reports_the_live_picture(self):
        ...
        with self._at("2026-03-09 08:00:00"):
            ...
            data = self.Analytics.get_sla_dashboard_data()
    ...
    def test_14_document_health_never_counts_an_attachment_as_evidence(self):
        ...
        data = self.Analytics.get_document_health_data()
        ...
        line.with_user(self.user_cert).write({"notes": "Illegible scan."})
